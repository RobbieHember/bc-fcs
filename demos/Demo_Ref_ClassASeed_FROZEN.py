'''
FROZEN VERSION USED IN THE 2024 INFO NOTE
'''
...
meta['Paths'][pNam]={}
meta['Paths'][pNam]['Data']=r'C:\Data\BCFCS\Demo_Ref_ClassASeed'
meta=cbu.ImportProjectConfig(meta,pNam)
meta['Graphics']['Print Figures']='On'
meta['Graphics']['Print Figure Path']=r'C:\Users\rhember\OneDrive - Government of BC\Figures\Reforestation\Demo' + '\\' + pNam
...
#%% Import data
pNam='Demo_RefGenGain'
pth=r'C:\Data\BCFCS\Demo_RefGenGain\Inputs\Metadata.pkl'
meta=gu.ipickle(pth)
...
# Interior
d=gu.ipickle(r'C:\Data\BCFCS\Demo_RefGenGain\Outputs\Scenario0002\Data_Scn0002_Ens0001_Bat0001.pkl')
z={}
...
# Coast
d=gu.ipickle(r'C:\Data\BCFCS\Demo_RefGenGain\Outputs\Scenario0004\Data_Scn0004_Ens0001_Bat0001.pkl')
z={}
for k in d['C_M_DistByAgent'].keys():
	idx=d['C_M_DistByAgent'][k]['idx']
	cd=cbu.lut_n2s(meta['LUT']['Event'],k)[0]
...
udem.PlotVolume(meta,pNam,mos,cnam=cnam,t0=t0,t1=t1,operSpace='Mean',ScenarioLabels=['Baseline scenario','Action scenario'],LegendLoc='upper right',FigSize=[12,6],TextDelta=td,FillDelta='On')
...
# plt.close('all'); fig,ax=plt.subplots(1,3,figsize=gu.cm2inch(16,5.75))
# v='V_MerchTotal'
# y1=mos[pNam]['Scenarios'][iB][operS][v]['Ensemble Mean'][iT,iPS,iSS,iYS]
# y2=mos[pNam]['Scenarios'][iP][operS][v]['Ensemble Mean'][iT,iPS,iSS,iYS]
# ymx=1.2*np.max([np.max(y1),np.max(y2)])
# ax[0].fill_between(tv[iT],y1,y2,color=[0.8,1,0.3],alpha=0.5,linewidth=0)
# ax[0].plot(tv[iT],y1,'-',color=[0,0,0],lw=1.25,label='Baseline')
# ax[0].plot(tv[iT],y2,'--',color=[0.6,0.9,0],lw=1.25,label='Action')
# ax[0].legend(loc='lower right',frameon=False,facecolor=None)
# be_d=(y2-y1)/y1*100
# for yr in td['Year']:
# 	iT2=np.where(tv[iT]==yr)[0]
# 	if be_d[iT2]>0:
...
# 	else:
# 		tx='-' + str(np.round(be_d[iT2[0]],decimals=1)) + '%'
# 	ax[0].text(tv[iT][iT2],1.05*y2[iT2],tx,fontsize=7,fontweight='bold',ha='center',color=[0.3,0.45,0])
# ax[0].set(ylabel=r'Net merch. stemwood volume (m$^3$ ha$^-$$^1$)',xlabel=r'Time, years',ylim=[0,ymx],xlim=[np.min(tv[iT]),np.max(tv[iT])]);
# ax[0].yaxis.set_ticks_position('both'); ax[0].xaxis.set_ticks_position('both'); ax[0].tick_params(length=meta['Graphics']['gp']['tickl'])
...
# y2=mos[pNam]['Scenarios'][iP][operS][v]['Ensemble Mean'][iT,iPS,iSS,iYS]
# ymx=1.2*np.max([np.max(y1),np.max(y2)])
# ax[1].fill_between(tv[iT],y1,y2,color=[0.8,1,0.3],alpha=0.5,linewidth=0)
# ax[1].plot(tv[iT],y1,'-',color=[0,0,0],lw=1.25,label='Baseline')
# ax[1].plot(tv[iT],y2,'--',color=[0.6,0.9,0],lw=1.25,label='Action')
# be_d=(y2-y1)/y1*100
# for yr in td['Year']:
# 	iT2=np.where(tv[iT]==yr)[0]
# 	if be_d[iT2]>0:
...
# 	else:
# 		tx='-' + str(np.round(be_d[iT2[0]],decimals=1)) + '%'
# 	ax[1].text(tv[iT][iT2],1.05*y2[iT2],tx,fontsize=7,fontweight='bold',ha='center',color=[0.3,0.45,0])
# ax[1].set(ylabel=r'Forest ecosystem (tC/ha)',xlabel=r'Time, years',ylim=[0,ymx],xlim=[np.min(tv[iT]),np.max(tv[iT])]);
# ax[1].yaxis.set_ticks_position('both'); ax[1].xaxis.set_ticks_position('both'); ax[1].tick_params(length=meta['Graphics']['gp']['tickl'])
...
# y2=mos[pNam]['Scenarios'][iP][operS][v]['Ensemble Mean'][iT,iPS,iSS,iYS]
# ymx=1.2*np.max([np.max(y1),np.max(y2)])
# ax[2].fill_between(tv[iT],y1,y2,color=[0.8,1,0.3],alpha=0.5,linewidth=0)
# ax[2].plot(tv[iT],y1,'-',color=[0,0,0],lw=1.25,label='Baseline')
# ax[2].plot(tv[iT],y2,'--',color=[0.6,0.9,0],lw=1.25,label='Action')
# be_d=(y2-y1)/y1*100
# for yr in td['Year']:
# 	iT2=np.where(tv[iT]==yr)[0]
# 	if be_d[iT2]>0:
...
# 	else:
# 		tx='-' + str(np.round(be_d[iT2[0]],decimals=1)) + '%'
# 	ax[2].text(tv[iT][iT2],1.05*y2[iT2],tx,fontsize=7,fontweight='bold',ha='center',color=[0.3,0.45,0])
# ax[2].set(ylabel=r'Products (tC/ha)',xlabel=r'Time, years',ylim=[0,ymx],xlim=[np.min(tv[iT]),np.max(tv[iT])]);
# ax[2].yaxis.set_ticks_position('both'); ax[2].xaxis.set_ticks_position('both'); ax[2].tick_params(length=meta['Graphics']['gp']['tickl'])

# #ax[0].annotate('Harvest',xy=(meta[pNam]['Project']['Year Project']-36,1000),xytext=(meta[pNam]['Project']['Year Project']-36,1200),arrowprops={'color':'black','arrowstyle':'->'},ha='center');
# gu.axletters(ax,plt,0.05,0.9,FontColor=meta['Graphics']['gp']['cla'],LetterStyle=meta['Graphics']['Modelling']['AxesLetterStyle'],FontWeight=meta['Graphics']['Modelling']['AxesFontWeight'])
# plt.tight_layout()
#gu.PrintFig(meta['Graphics']['Print Figure Path'] + '\\SummaryTS','png',900)
